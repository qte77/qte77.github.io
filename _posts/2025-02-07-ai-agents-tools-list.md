---
layout: post
title: AI Agents Tools List
excerpt: A categorized reference list of AI agent frameworks, research agents, benchmarking tools, execution tracers, and AI-enhanced workflow platforms available as of early 2025.
categories: [ml, ai, agents, tools,list]
---

# AI Agents Tools List

## Lists

- [300+ agent frameworks](https://github.com/slavakurilyak/awesome-ai-agents)
- [500+ curated list of use cases](https://github.com/ashishpatel26/500-AI-Agents-Projects)

## Frameworks

- [abacus.ai](https://abacus.ai/)
- [Agentforce 2.0 (Salesforce)](https://www.salesforce.com/agentforce/)
- [Amazon Bedrock AI Agent framework](https://aws.amazon.com/bedrock/agents/)
- [AutoGen (Microsoft)](https://github.com/microsoft/autogen)
- [AutoGPT](https://agpt.co/)
- [CrewAI](https://www.crewai.com/)
- [DSpy.ai (Stanford)](https://dspy.ai/)
- [LangGraph (LangChain)](https://langchain-ai.github.io/langgraph/)
- [Lyzr](https://www.lyzr.ai/)
- [pydantic-ai](https://ai.pydantic.dev)
- [smolagents (Hugging Face)](https://smolagents.org/)

## Research-Agents

- [Open DeepResearch (Huggingface)](https://huggingface.co/blog/open-deep-research)
- [DeepResearch (OpenAI)](https://openai.com/index/introducing-deep-research/)
- [DeepResearch (Google)](https://blog.google/products/gemini/google-gemini-deep-research/)
- [RD-Agent (Microsoft)](https://github.com/microsoft/RD-Agent)
- [CO-STORM (Stanford)](https://storm.genie.stanford.edu/)

## Benchmarks

- [AgentNeo](https://github.com/raga-ai-hub/agentneo)
- [AgentOps (Agency)](https://www.agentops.ai/)
- [AutoGenBench (Microsoft)](https://aka.ms/agbench)
- [Mosaic AI Agent Evaluation](https://docs.databricks.com/en/generative-ai/agent-evaluation/index.html)
- [RagaAI-Catalyst](https://github.com/raga-ai-hub/RagaAI-Catalyst)

## Tracing

- [Postman AI Builder](https://www.postman.com/ai-on-postman/postman-ai-agent-builder/overview)
- [W&B Weave - Weights & Biases](https://wandb.ai/site/weave/)

## AI-enhanced Workflows

- [BotPress](https://botpress.com)
- [gumloop.com](https://www.gumloop.com)
- [Langflow](https://www.langflow.org)
- [Mulesoft (Salesforce)](https://www.mulesoft.com)
- [n8n.io](https://n8n.io/)
- [Postman AI Builder](https://www.postman.com/ai-on-postman/postman-ai-agent-builder/overview)
- [Relay.app](https://Relay.app)
- [Rivet](https://rivet.ironcladapp.com/)
- [Vellum](https://www.vellum.ai/)
